...
@Component({
  ...
  standalone: true,
  imports: [RouterLink, RouterLinkActive, Icon, ParametresModal],
  ...
  template: `
    ...
    >
      ...
      <!-- Sélecteur de Structure Multi-Tenant (UX SaaS) -->
      <div class="relative mb-6 shrink-0">
        @if (activeMembership(); as active) {
          ...
          >
            <!-- Badge Initiales de la Structure -->
          ...
          <!-- Dropdown / Popover de Changement de Structure -->
        ...
        <nav class="flex flex-col gap-1">
          @for (item of navItems(); track item.label) {
            <div class="relative">
              ...
              <!-- Item principal avec sous-menu (Accordéon) -->
              @else {
                <button
                  type="button"
...
/>
                ...
                <!-- Arborescence sous-menu (Lignes d'indentation) -->
                ...
                <!-- Popover Sous-menu quand la sidebar est réduite -->
                @if (isSubmenuOpen(item.label) && collapsed()) {
      ...
      <!-- Carte Profil Utilisateur (En bas de sidebar) -->
      <div class="relative mt-3 shrink-0 border-t border-line pt-3">
  @if (currentUser(); as user) {
    <button
      type="button"
      (click)="toggleProfileMenu()"
      class="flex w-full items-center gap-2.5 rounded-xl border border-line bg-surface p-2 transition-all hover:bg-surface-muted cursor-pointer"
      [class.justify-center]="collapsed()"
    >
...
export class Sidebar {
  ...
  // Suivi de l'état d'ouverture des sous-menus
